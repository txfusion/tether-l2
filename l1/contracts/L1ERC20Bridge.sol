...
import {IL2ERC20Bridge} from "@matterlabs/zksync-contracts/l1/contracts/bridge/interfaces/IL2ERC20Bridge.sol";
...
import {AllowListed} from "@matterlabs/zksync-contracts/l1/contracts/common/AllowListed.sol";
    ...
    ReentrancyGuard
{
    ...
    /// @notice >>>>>> (Q: Isn't this supposed to be a generalized bridge for all assets) <<<<<<<
    ...
    /// @param _l1Token Address of the allowed token on L1 
    function initialize(
        ...
        onlyNonZeroAddress(_l2TokenBeacon) 
        onlyNonZeroAddress(_governor) 
        onlyNonZeroAddress(_l1Token) 
        onlyNonZeroAddress(_l1Admin)
    {
        // ========== Root L1ERC20Bridge initializer ==========
        ...
        // ========== Tether specific inits ==========
        __BridgeableTokens_init(_l1Token, _l2TokenBeacon);
        __BridgingManagerUpgradeable_init(_l1Admin);
        __Ownable_init();
        transferOwnership(_l1Admin);
        // ===========================================
    }
    ...
    ) external payable whenDepositsDisabled onlySupportedL1Token(_l1Token) returns (bytes32 l2TxHash) {
    ...
    ) public payable whenDepositsEnabled onlySupportedL1Token(_l1Token) nonReentrant senderCanCallFunction(allowList) returns (bytes32 l2TxHash) {
    ...
    ) external nonReentrant onlySupportedL1Token(_l1Token) senderCanCallFunction(allowList) {
    ...
    function finalizeWithdrawal(
        ...
        bytes32[] calldata _merkleProof
    ) external nonReentrant whenWithdrawalsEnabled senderCanCallFunction(allowList) { // <--- Q: do we need `whenWithdrawalsEnabled` here?
