...
import { SingletonFactory__factory } from "../../typechain";
...
export async function deployViaCreate2(
  ...
  const bytecode = contractFactory.getDeployTransaction(...args, ethTxOptions)
    .data as ethers.utils.BytesLike;
...
export async function deployBytecodeViaCreate2(
  ...
  const create2Factory = SingletonFactory__factory.connect(
    create2FactoryAddress,
    deployWallet
  );
  ...
  const deployedBytecodeBefore = await deployWallet.provider.getCode(
    expectedAddress
  );
  ...
  const deployedBytecodeAfter = await deployWallet.provider.getCode(
    expectedAddress
  );
  if (ethers.utils.hexDataLength(deployedBytecodeAfter) == 0) {
    throw new Error(
      `Failed to deploy ${contractName} bytecode via create2 factory`
    );
  }
...
export interface DeployedAddresses {
  ...
  Bridges: {
    LegacyERC20BridgeImplementation: string;
...
export function deployedAddressesFromEnv(): DeployedAddresses {
  return {
    Bridgehub: {
      ...
      BridgehubImplementation: getAddressFromEnv(
        "CONTRACTS_BRIDGEHUB_IMPL_ADDR"
      ),
    },
    StateTransition: {
      StateTransitionProxy: getAddressFromEnv(
        "CONTRACTS_STATE_TRANSITION_PROXY_ADDR"
      ),
      StateTransitionImplementation: getAddressFromEnv(
    ...
    Bridges: {
      LegacyERC20BridgeImplementation: getAddressFromEnv(
